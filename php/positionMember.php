<?php header('Content-type:text/html;charset=UTF-8');
	...
	if(isset($order_column)){
	    ...
	    switch($i){
	        case 0: $orderSql = " order by Pid ".$order_dir; break;
	        case 1: $orderSql = " order by Pname ".$order_dir; break;
	        ...
	        case 7: $orderSql = " order by Dname ".$order_dir; break;
	        default: $orderSql = '';
	    }
	}
	...
	//定义过滤条件查询过滤后的记录数sql
	$sumSqlWhere =" where member.Position_id=position.id and member.Department_id=department.id and member.Position_id like '%".$search."%'";
	if(strlen($search)>0){
   ...
   //query data
	$totalResultSql = "select position.id as Pid,position.name as Pname, member.id as Mid,member.name as Mname,birth,sex,addr,department.name as Dname";
	...
	if(strlen($search)>0) {
		...
		while($row = mysql_fetch_array($dataResult)){
			$obj = array($row['Pid'],$row['Pname'],$row['Mid'],$row['Mname'],$row['birth'],$row['sex']);
			array_push($obj,$row['addr']);
			array_push($obj,$row['Dname']);
			array_push($data,$obj);		
		}	
	}else {
		//直接查询所有记录
